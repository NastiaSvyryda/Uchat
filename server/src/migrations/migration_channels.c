...
static void create_table_for_sqlite3(void) {
    ...
    asprintf(&sql, "CREATE TABLE IF NOT EXISTS %s (\n"
                         ...
                         model_channel_name_table(), model_user_name_table());
...
void migration_users(bool status) {
    if (mx_strcmp(model_channel_database(), "sqlite3") == 0 && status == true)
        create_table_for_sqlite3();
    else if (mx_strcmp(model_channel_database(), "sqlite3") == 0 && status == false)
